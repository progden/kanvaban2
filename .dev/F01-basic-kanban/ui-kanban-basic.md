...
### 進入與離開
- 從哪裡進來：`s-board` 的「開啟卡片詳情」操作；F02 `s-cards-by-assignee`（跨模組）的「開啟卡片詳情」操作
- 完成後去哪裡：儲存變更或新增留言後停留本畫面，對應欄位更新為最新內容；開啟負責人選取入口後導向 `s-card-assignee-picker`（F02），選取完成後回到本畫面，負責人欄位顯示更新後的名單；關閉回到進入前的畫面（`s-board` 或 F02 `s-cards-by-assignee`，跨模組）
...
- 關閉後回到進入前的畫面（`s-board` 或 F02 `s-cards-by-assignee`）；回到 `s-board` 時，看板交會格內容更新

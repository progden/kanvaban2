...
### 進入與離開
- 從哪裡進來：F07 s-canvas 畫面上方列的「看板時間」按鈕，所有看得到該 Board 的成員皆可點擊開啟（CR-014）
...
| `r-board-member`（F02，跨模組） | 看板時間目前值與狀態 | 不做得到（依 `uc-adjust-board-clock`／`uc-pause-resume-board-clock` 的角色定義僅 `r-board-owner`） |

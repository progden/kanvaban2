...
本則取代 OQ-T-14-fe-board-item-01（該則第三段引文非逐字，見 D-04）。前兩段引文與 OQ-T-14-fe-board-item-01 相同，不重複開立，僅第三段改為逐字版：
...
引用 `.dev/loops/ui-authoring-loop/.state/ui-authoring-open-questions.md` OQ-19「選項」欄第 2 項全文：
『「看板成員清單」改為 F07 `s-canvas` 上另一個獨立 `item`（本列採用），`s-board` 資料段移除該欄位；`uc-assign-card-owner-by-drag` 的拖曳來源（成員頭像）與拖曳目標（`s-board` 上的卡片）分屬 F07 兩個不同 `item`，跨 `item` 拖曳的機制 `spec-canvas-layout.md` 目前未定義，須留待 F07 補充或視為前端層級的技術實作細節』

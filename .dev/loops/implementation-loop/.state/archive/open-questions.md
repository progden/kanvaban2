...
狀態：**已解除（2026-09-19，人工決策，採選項 A：併入 T-03-be-card）**。
解除說明：(a) 這不是規格缺口：兩個 uc 的結果 spec 已定義（`uc-delete-swimlane` post 逐字『"若該 `swimlane` 內有 `card`，一併被刪除"』；`uc-delete-stage` post 逐字『"若該 `stage` 內有 `card`，`card.stage` 更新為使用者選擇的目的 `stage`"』），「確認／選目的 Stage」的互動由 `ui-kanban-basic.md` 兩個 delete dialog 承接。【推論】「有卡片」不是 `fail`，是需要更多輸入（已確認／目的 `stage`），所以不在 `fail` 裡；後端第一次回 409 附卡片數、前端確認後帶參數重送，是介面設計，不需要改 spec。(b) 人工決議由 T-03-be-card 一起做——同一個 feature 的修改不要分散到另一個任務；`tasks.md` T-03 的產出範圍與備註已補上（協調流程、刪除端點參數、真的 `CardLookupPort`、四個替身步驟改打正式端點）。(c) 為了避免 T-03 與 T-04 並行時同改 T-02 的「BoardController」／「BoardApplicationService」造成程式碼合併衝突，T-04 的依賴加上 T-03，兩者依序執行。

...
| T-02-be-board | F01 `board` Aggregate（`board`＋`swimlane`＋`stage`，含 `stage.role` START/DONE 唯一性） | T-01-be-user（`board.created-by`） | doing | 2026-09-18 Dev 第 1 輪交出：`kanban-core`（`Board`／`Swimlane`／`Stage`／`ActivityRecord`／`CardLookupPort`）＋ `kanban-spring`（persistence／application／web）＋ 2 個 feature 檔（swimlane/stage 管理，共 15 個 Scenario）全過；`Board.create` 預設 Swimlane/Stage 名稱依 OQ-IMPL-12 推論；`r-board-owner` 權限檢查未加（依賴 T-04，見 OQ-IMPL-13）；`uc-create-board` 只實作 `board` 部分，不含 `board-membership`（見 OQ-IMPL-14）。 |

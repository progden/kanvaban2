...
| T-04-be-board-membership | F02 `board-membership` ＋ ActivityRecord | T-02-be-board、T-01-be-user | CR-009（2026-09-19）：一併補上 `uc-create-board` 新 Scenario「建立 Board 後帶有預設的 Swimlane 與 Stage」的 Cucumber step definition（「Board.create」行為不用改，現況已符合；見 `archive/open-questions.md` OQ-IMPL-14 解除說明）。完成並驗收後，人工把 `.dev/CR.md` CR-009 改「處理完成」並清掉 spec 的 `@added @wip` |

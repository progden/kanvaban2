...
| T-11-fe-auth | `s-login`、`s-signup` | T-10-fe-shell | review-pending | 2026-09-18 Dev 第 2 輪處理完 D-05～D-07（見 `decision-log.md`）：D-05 已登記 OQ-IMPL-13（`ui-user-membership.md` 第 45 行「不觸發 `uc-create-user`」與 spec `pre p2`／`fail p2`／Scenario 矛盾），程式碼未變動；D-06／D-07 已補測試斷言，產品程式碼未變動。OQ-IMPL-12、OQ-IMPL-13 皆待人工處理，不影響操作／驗收條件行為 |

...
| T-04-be-board-membership | F02 `board-membership` ＋ ActivityRecord | T-02-be-board、T-01-be-user | CR-009（2026-09-19）：一併補上 `uc-create-board` 新 Scenario「建立 Board 後帶有預設的 Swimlane 與 Stage」的 Cucumber step definition（「Board.create」行為不用改，現況已符合；見 `archive/open-questions.md` OQ-IMPL-14 解除說明）。完成並驗收後，人工把 `.dev/CR.md` CR-009 改「處理完成」並清掉 spec 的 `@added @wip` **T-02 交接（OQ-IMPL-15／16，2026-09-19 人工確認）**：(1) 替 T-02 的九個結構調整端點（`uc-add-swimlane`…`uc-set-stage-role`，roles 皆 `r-board-owner`）補上 Owner 檢查，並實作 `uc-reject-structure-change-by-member` 的 Scenario；(2) `uc-create-board` 要在**同一次交易**內建立 Owner 的 `board-membership`（不可以只成功一半），補上 Scenario「建立 Board 的人自動成為 Owner」的 Cucumber 驗收；(3) 怎麼接（直接改「BoardApplicationService.createBoard」或另開協調層）是實作細節，由 Dev 決定，會約束後續任務的話開 ADR。允許改動 T-02 已合併的 application／web／測試檔，只限這三點需要的部分 |

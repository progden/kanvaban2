...
# ---------- 啟動前檢查任務清單：依賴欄不可以出現不存在的任務 ID ----------
# 依賴用 grep 'T-xxx' 從第 3 欄抓；說明文字只要寫到「T-02 的…」這種縮寫，就會多出一個永遠
# 不會 done 的依賴，那個任務（與它所有下游）永遠不會被派工，而且沒有任何錯誤訊息。
bad_deps=0
for id in $(task_ids); do
  for dep in $(task_deps "$id"); do
    if ! grep -q "^| $dep |" "$LEDGER"; then
      echo "任務清單錯誤：$id 的依賴欄出現不存在的任務 ID「$dep」（說明文字不要寫 T-xx 縮寫）" >&2
      bad_deps=1
    fi
  done
done
[ "$bad_deps" -eq 0 ] || exit 1

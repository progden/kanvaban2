#!/usr/bin/env bash
# spec 遷移 loop：每一輪都是全新的 claude process（zero context），狀態只存在檔案與 git 歷史。
#
# 每一輪：
...
#   4. 由 verify-spec-migration.sh 外部驗證，不採信 agent 自己的回報。
#
...
KICKOFF="$LOOP/spec-migration-kickoff-prompt.md"
REVIEW_PROMPT="$LOOP/spec-migration-review-prompt.md"
VERIFY="$LOOP/verify-spec-migration.sh"
...
LOOP_BRANCH="${LOOP_BRANCH:-loop/spec-migration}"
...
# baseline：loop 第一次啟動時的 HEAD，cr-check 一律以它為 base（不是每輪的 base）
if [ ! -s "$BASELINE" ]; then
...
while true; do
  ...
  if [ -z "$first" ]; then
    ...
    echo "請查看 $LOOP/spec-migration-open-questions.md 與任務清單中 blocked 的任務，處理後將其改回 todo 再重新啟動。"
    break
  fi
  ...
  if [[ "$first" == G* ]] && [ ! -f "$GATES/$first.approved" ]; then
    ...
    if [ "$reviewed_gate" = "$first" ]; then
      # 剛審查完且下一個仍是關卡，代表審查沒有留下待修的 D-xx
      if [ "$AUTO_APPROVE_GATES" != 1 ]; then
        log "等待人工關卡 $first：確認審查紀錄（$LOOP/spec-migration-review.md）後執行"
    ...
    # 下一個不是關卡（例如先修 D-xx），修完回到關卡時要重新審查
  ...
  # 請在專用分支執行；每一輪結果仍由 verify-spec-migration.sh 外部驗證。
  set +e

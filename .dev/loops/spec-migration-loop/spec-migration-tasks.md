...
| T3.02 | blocked | `.dev/CR.md` 回填 CR-001～CR-004 的影響 ID（依各 spec 掛該 `@CR-` 的 Scenario 所屬 uc 與相關實體）；CR-005 影響 ID 列出本次遷移改動到的全部實體與 uc；跑 `cr-check` 到通過 | `crcheck(CR-005)=0`；`errors(all)=0` | T3.01 |

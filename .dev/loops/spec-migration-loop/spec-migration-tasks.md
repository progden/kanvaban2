...
| D-11 | todo | [F02] `.dev/CR.md` 的 CR-002 影響 ID 移除 `uc-assign-card-owner-by-drag`(新增)（只改這一處，CR-001、CR-003～CR-005 與明細不動）。依據：F02 變更紀錄 2026-09-13「（原票號 F05）」那列明寫兩條拖曳 Scenario「本檔尚未進入開發，可直接補上，不需開 CR」，不是 CR-002 帶進來的；這兩條 Scenario 也沒有掛 `@CR-002`。不需新增 OQ | `crcheck(CR-005)=0`；`errors(all)=0`；`.dev/CR.md` 的 CR-002 那列不含 `uc-assign-card-owner-by-drag` | — |

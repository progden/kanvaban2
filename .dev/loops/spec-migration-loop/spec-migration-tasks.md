...
| T2.13 | done | [F03] 遷移程序 10～11 與收尾，F03 全檔 0 error | `errors(F03)=0` | T2.12 |
...
| T2.18 | done | [F05] 「人員工作量檢視」usecase 區塊＋tag＋Aggregate 註解（拖曳追加負責人的 Scenario 屬於哪個 uc，依 F02「卡片負責人指派」已定義的 uc 判斷，不重複定義） | `errors(F05#人員工作量檢視)=0` | T2.17 |
...
| G2 | done | 關卡：審查輪做 `llm-review.md` L-01、L-02、L-05、L-06 抽查（每模組至少一個 uc），偏差開 D-xx 或記入審查紀錄 | 腳本建立 `runtime/gates/G2.approved` | T3.03 |
| T3.04 | done | 最終確認並建立 `runtime/DONE` | `errors(all)=0`；`crcheck(CR-005)=0`；任務清單除本任務外無 todo／doing／blocked／proposed | G2 |
...
| D-06 | done | [F03] 補完 T2.13 漏做的遷移程序 10（`design.md` 改實際檔名）：F03 變更紀錄 2026-09-13「開發完成」那列摘要末尾的「詳見 `design.md`「實作狀態」段落」改為「詳見 `design-kanban-widgets.md`「實作狀態」段落」（`design-kanban-widgets.md` 確有「## 實作狀態」段落）；其他文字、日期、類型欄都不動，gherkin 不動 | `errors(F03)=0`；F03 spec 不再含「`design.md`」字樣，且含「`design-kanban-widgets.md`「實作狀態」」 | — |
| D-07 | done | [F04] 把 T2.15 的 `uc-guard-clock-monotonicity` 建模決定補記為 OQ（只改 OQ 檔與 F04 `## 待釐清`，usecase 區塊、gherkin、Aggregate 註解都不動）。在 OQ 檔末尾追加一列（模組 F04；情況：單調性檢查是所有 `board` 底下寫入的共同前置條件，原本應是 F01 `uc-add-card` 等寫入 uc 的 fail，但 GH-01 限定 `@uc-` 必須屬於同一 Feature、UC-06 要求每個 uc 有成功 Scenario，因此另立 `uc-guard-clock-monotonicity`，並把 When 為「調整看板時間＋建立卡片」的「把看板時間調整到未來後建立卡片」Scenario 歸給它當成功案例，而不是歸給 `uc-adjust-board-clock`；此 uc 的 `crud` 只有 `board: U`，沒有列出實際建立的 `card`，沿用原 Aggregate 註解；採用：維持現狀；依據：F04 兩個單調性 Scenario、PDCA Iteration 28、OQ-06 的同類處理），F04 `## 待釐清` 末尾追加一行指向該 OQ（格式比照 F02 的「- OQ-xx：…（見 `.dev/loops/spec-migration-loop/spec-migration-open-questions.md`）」，原有兩行保留） | `errors(F04#看板時間管理)=0`；OQ 檔最後一列模組為 F04、內容提到 `uc-guard-clock-monotonicity` 與 GH-01；F04 spec 的 `## 待釐清` 段落有一行含該 OQ 編號 | — |
| D-08 | done | [F05] 補回 `uc-drag-assign-card-owner` post 第二句的活動紀錄限制，讓它跟本檔待釐清的定案、F02 `uc-assign-card-owner-by-drag` 一致（只改 usecase 區塊，gherkin、Aggregate 註解、`crud` 都不動）。採用「依本檔待釐清第一行（拖曳到已經是負責人的卡片：靜默忽略、不產生活動紀錄）補上」：把「若該成員已經是 `card.assignees` 成員，`card.assignees` 維持不變」改為「若該成員已經是 `card.assignees` 成員，`card.assignees` 維持不變，且不產生新的活動紀錄」；第一句不加活動紀錄（本檔 Scenario 與待釐清都沒有寫），也不新增 OQ | `errors(F05#人員工作量檢視)=0`；F05 spec 含「`card.assignees` 維持不變，且不產生新的活動紀錄」 | — |
| D-09 | done | [F06] 移除 T2.22 在變更紀錄 2026-09-13「開發完成」那列摘要開頭加上的「（原票號 F06）」（只改這一處；日期、票號欄空白、類型「開發完成」、其餘摘要文字與 CR-005 列都不動，gherkin 不動）。採用「原表格沒有票號欄就不加原票號註記」：F06 舊變更紀錄是兩欄（日期／內容），沒有任何票號，「F06」是執行輪比照 F05 自行補上的，等於捏造原文沒有的資訊；摘要保留「開發完成：」前綴。不需新增 OQ | `errors(F06)=0`；F06 spec 不含「原票號 F06」字樣，且含「開發完成：「kanban-spring」新增「query.featurecrboard.FeatureCrBoardCalculator」」 | — |
| D-10 | done | [F01] 修正兩處 usecase 措辭造成的指涉不清（只改 usecase 區塊，gherkin、Aggregate 註解、`crud` 都不動，不新增 OQ）。(1) `uc-set-stage-role` post 第二句的「該 `stage`」讀起來像是被設定的那個 Stage，跟「設定 Stage 角色」Scenario 的 Then（被設定的「進行中」變 START、原本持有的「待辦」變回 NONE）相反。採用「明寫是原本持有該角色的另一個 Stage」：改為「若新角色為 START 或 DONE，且 `board` 中原本已有其他 `stage` 持有該角色，該其他 `stage` 的角色自動變回 NONE」。(2) `uc-move-card-stage` 的 `pre.p1`「`card` 存在於指定的 `stage`」分不出是來源還是目的；依 Scenario Given「位於 Stage "待辦"」，改為「`card` 存在，且位於來源 `stage`」 | `errors(F01#Stage（階段）管理)=0`；`errors(F01#Card（卡片）編輯)=0`；F01 spec 含「該其他 `stage` 的角色自動變回 NONE」與「`card` 存在，且位於來源 `stage`」 | — |
| D-11 | done | [F02] `.dev/CR.md` 的 CR-002 影響 ID 移除 `uc-assign-card-owner-by-drag`(新增)（只改這一處，CR-001、CR-003～CR-005 與明細不動）。依據：F02 變更紀錄 2026-09-13「（原票號 F05）」那列明寫兩條拖曳 Scenario「本檔尚未進入開發，可直接補上，不需開 CR」，不是 CR-002 帶進來的；這兩條 Scenario 也沒有掛 `@CR-002`。不需新增 OQ | `crcheck(CR-005)=0`；`errors(all)=0`；`.dev/CR.md` 的 CR-002 那列不含 `uc-assign-card-owner-by-drag` | — |
| D-12 | done | [F02] `uc-remove-member` 的 `pre` 補上「操作者是 Owner」，跟同 Feature 的 `uc-invite-member`、`uc-change-member-role` 一致（`roles` 已是 `r-board-owner`；成功 Scenario「還有其他 Owner 時，可以移除其中一位 Owner」由 Owner "user1" 操作）。採用「只加 pre、不加 fail」（比照 `uc-change-member-role` 的 p1，Member 被拒絕另由 OQ-06 的拒絕類 uc 承接）：新增 `p3: "移除者是該 `board` 的 Owner"`；p1、p2、post、fail、gherkin 都不動，不新增 OQ | `errors(F02)=0`；F02 spec 含「移除者是該 `board` 的 Owner」 | — |
| D-13 | done | [F04] 補齊「看板時間管理」兩個 uc 的 post，讓它反映 Scenario 的 Then（只改 usecase 區塊，gherkin、Aggregate 註解、`crud` 都不動，不新增 OQ）。(1) `uc-pause-resume-board-clock` post 第一句「`board` 的看板時間狀態切換為 PAUSED 或 REALTIME」沒寫出「暫停看板時間」「恢復看板時間」兩個 Scenario 驗證的時間行為，拆成兩句：「暫停時，`board` 的看板時間狀態切換為 PAUSED，看板時間停在暫停當下的時間」與「恢復時，`board` 的看板時間狀態切換為 REALTIME，看板時間從暫停時的時間繼續隨系統時間前進」。(2) 兩個 uc 的活動紀錄那句沒寫出 Then 驗證的說明內容；保留「包含操作人與操作時間」（CR-001 慣例），`uc-adjust-board-clock` 改為「該操作被記錄為 `board` 的一筆活動紀錄，說明看板時間被調整為指定時間，包含操作人與操作時間」，`uc-pause-resume-board-clock` 改為「該操作被記錄為 `board` 的一筆活動紀錄，說明看板時間已暫停或已恢復，包含操作人與操作時間」 | `errors(F04)=0`；F04 spec 含「看板時間停在暫停當下的時間」「看板時間從暫停時的時間繼續隨系統時間前進」「說明看板時間被調整為指定時間」「說明看板時間已暫停或已恢復」 | — |
| D-14 | done | [F05] 修正 usecase 區塊下方說明段與 D-08 的不一致：說明段寫「「pre」「post」只依本 Feature 的 Scenario 步驟推導（見 OQ-09）」，但 D-08 已依本檔待釐清的定案補上「且不產生新的活動紀錄」。採用「保留 D-08 的 post，改說明段」：改為「「pre」「post」依本 Feature 的 Scenario 步驟與本檔待釐清的定案推導（見 OQ-09）」；usecase 區塊、gherkin 都不動，不新增 OQ | `errors(F05)=0`；F05 spec 含「依本 Feature 的 Scenario 步驟與本檔待釐清的定案推導」且不含「只依本 Feature 的 Scenario 步驟推導」 | — |

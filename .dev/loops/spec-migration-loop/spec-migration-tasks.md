...
| D-06 | todo | [F03] 補完 T2.13 漏做的遷移程序 10（`design.md` 改實際檔名）：F03 變更紀錄 2026-09-13「開發完成」那列摘要末尾的「詳見 `design.md`「實作狀態」段落」改為「詳見 `design-kanban-widgets.md`「實作狀態」段落」（`design-kanban-widgets.md` 確有「## 實作狀態」段落）；其他文字、日期、類型欄都不動，gherkin 不動 | `errors(F03)=0`；F03 spec 不再含「`design.md`」字樣，且含「`design-kanban-widgets.md`「實作狀態」」 | — |

...
- 上一輪任務：T1.08（定案 `s-card-detail`，類型改為表單，對應 `uc-edit-card`／`uc-add-comment`）
...
- 待注意：`ui-check`（不帶 `--spec`）25 error（開工前 38，下降因 `s-card-detail` 補齊八段消除原本的 DS-02／DS-04 缺口）；皆未較開工前增加；`verify-quotes.py` 通過。

...
- 上一輪任務：D-01（修正 `s-swimlane-list` 操作表 REF-07：「無 `fail` 定義」誤用反引號，已改為「無 fail 定義」）
...
- 待注意：`uc-delete-swimlane` 的 post 是「Swimlane 內的卡片一併被刪除」而非轉移到其他泳道，故 `s-swimlane-delete-dialog` 沒有「目的泳道選擇」欄位；`ui-check` 全檔 error 80→78（D-01 修正 REF-07 反引號誤用）

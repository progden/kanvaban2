...
- 上一輪任務：D-13——確認 `s-signup`／`s-login` 段落無殘留 `⚠️`、「待確認事項」皆為「（無）」後，狀態行由「討論中」改為「已定案」；純文字改寫，`ui-check` error 數未變（39）
- 下一個任務：T2.08（[F02] 定案 `s-card-assignee-picker`：對應 `uc-set-card-assignees`／`uc-list-card-assignee-candidates`／`uc-view-card-assignees`；嵌入 F01 `s-card-detail`，需寫明從該畫面進入、完成回到該畫面）
- D-10 仍列 `doing`：其整檔 `ui-check=0` 驗收條件依賴 T2.05～T2.10 全部完成，內容本身已完成，待 T2.11 收尾任務整份歸零後可直接改 `done`，不需再動內容
- 已完成的模組：F01（8 個畫面全部定案，`ui-check` 0 error）；F02 已定案 `s-signup`／`s-login`／`s-board-list`／`s-board-create-dialog`／`s-board-delete-dialog`；`s-member-management` 內容完整（D-11 已修正業務結果問題）但因入口未定（OQ-22）維持「討論中」；`s-board-delete-dialog` 因入口未定（OQ-26）維持「討論中」；`s-card-assignee-picker`／`s-cards-by-assignee`／`s-activity-log` 骨架未討論
- 已定義的共用 ID：無新增

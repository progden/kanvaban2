...
- 上一輪任務：補 PDCA Iteration 31（D-11）遺漏的 ui-check 結果行——上一輪驗證 FAIL，本輪只追加 Iteration 32 補齊該行，未改動任何 ui 檔內容；D-11 本身（`6292049`）維持已完成
...
- 待注意：PDCA 每則 Check 段落務必貼 `./scripts/ui-check <本輪檔案> --spec ".dev/F[0-9][0-9]-*/spec-*.md"` 的最後一行（`N error(s), M warning(s)`），漏貼會讓 verify 判 FAIL（本輪教訓）；跑該指令務必加 `--spec` 選項；反引號只給 entity／`entity.attr`／`r-`／`uc-`／`ev-`／`s-` 六種 ID 用。

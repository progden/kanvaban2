...
| D-16 | todo | [F02] 修正 `ui-user-membership.md` 的 `s-card-assignee-picker` 引用 post 時與原文不符：「狀態」段「空資料」寫「卡片目前沒有指派負責人時，依 `uc-view-card-assignees` post，候選清單全部維持未勾選」，但該 post 原文只有『沒有指派負責人的 `card`，「負責人」欄位顯示為未指派，卡片縮圖不顯示負責人資訊』，沒有提到候選清單的勾選狀態。「全部未勾選」是從畫面推論出來的，不能掛在 post 名下。處理：刪掉這句的「依 `uc-view-card-assignees` post」，或改寫成原文實際描述的內容（「已選負責人」顯示為未指派）。另外，「儲存變更」的「成功後」寫「卡片縮圖同步顯示」，但卡片縮圖在 F01 `s-board`，不在本畫面，也不在回到的 `s-card-detail`。這段要刪掉，或改寫成「回到 `s-board` 時卡片縮圖顯示更新」這類寫法，並寫明是在哪個畫面 | `s-card-assignee-picker` 段落（`## s-card-assignee-picker` 到下一個 `## s-`）的「空資料」行不同時包含「`uc-view-card-assignees` post」和「未勾選」；操作表「儲存變更」列不含「卡片縮圖同步顯示」；`ui-check(.dev/F02-user-membership/ui-user-membership.md)=0`（帶 `--spec` 全部模組，並把全部 ui 檔一起檢查） | — |

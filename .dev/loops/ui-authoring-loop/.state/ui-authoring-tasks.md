...
| D-13 | todo | [F02] `ui-user-membership.md` 的 `s-signup`、`s-login` 標題下仍是「狀態：討論中」，但 D-10 已經把 OQ-11／OQ-12／OQ-13 的 ⚠️ 全部移除，兩個畫面八段齊全、「待確認事項」都是「（無）」、行號範圍內沒有 error。依 `ui-convention.md` 的狀態定義，沒有待確認事項的畫面不應維持「討論中」。先確認兩個畫面的行號範圍內沒有任何 `⚠️`，再把狀態改成「已定案」；如果發現仍有未解決的事項，就維持「討論中」，並在「待確認事項」補上對應的 ⚠️ 與 OQ 編號 | `s-signup` 與 `s-login` 的狀態行是「狀態：已定案」，且兩個畫面段落都不含 `⚠️`；或者狀態維持「討論中」，但「待確認事項」至少有一條 ⚠️ 並附 OQ 編號；`ui-check(.dev/F02-user-membership/ui-user-membership.md)` 的 error 數不增加（審查時為 39） | — |

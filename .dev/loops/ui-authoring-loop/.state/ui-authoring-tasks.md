...
| T1.10 | blocked | [F01] 收尾：`ui-check .dev/F01-basic-kanban/ui-kanban-basic.md` 0 error；過一遍 `DS-06`（寫入 uc 有沒有被引用）／`DS-07`（畫面有沒有被導向）warn，沒理由的回頭補 | `ui-check(.dev/F01-basic-kanban/ui-kanban-basic.md)=0`；PDCA 本則列出 `DS-06`／`DS-07` warn 清單與處理結果 | T1.09 |

...
| 固定字串 | 層級 | 出現次數 | 腳本用途 |
|------|------|------|------|
| `狀態：` | 第一個 H2 之前的一行 | 恰好 1 | 值為「草稿」或「定稿」，判定規格是否已定稿（需要走 CR） |
...
- **狀態行**：第一個 H2 之前一行 `狀態：草稿` 或 `狀態：定稿`（固定字串，必填）。「定稿」代表規格已鎖定、交付工程排期，之後的改動都要走 CR（第 5、6 節與 `cr-convention.md`）；首版交付時把「草稿」改成「定稿」，並在變更紀錄加一列「首版定稿」，這一步本身不用 CR。是否真的有人在開發這個模組是另一件事（衍生狀態，見 `cr-convention.md` §1.1），不寫在這一行。
...
- usecase 區塊不掛 tag。規格定稿後要改 usecase 區塊，一律開 CR（`cr-convention.md` 第 1 節），並在 CR 的影響範圍列出該 `uc-` ID；改動內容以 PR diff 為準。
...
- Given / When / Then / And 步驟不得出現「點擊、輸入、拖曳、顯示、畫面、按鈕、視窗、對話框、提示我」（建議新增檢查 GH-10，error）。
...
規格一旦定稿，之後的新增與修改都必須掛 tag。tag 寫在 Aggregate 註解之前、獨立一行。
...
| `@added` | 規格定稿後新增的 Scenario | 驗收通過後移除 |
...
| 定稿後的新增 / 變更都掛了 `@CR-xxx` 與狀態 tag；開發中模組被 diff 動到的 Scenario 也都有 `@CR-xxx` | GH-05 |
...
- [ ] Scenario 全文不含 UI 操作（禁字由 GH-10 擋，這裡看語意上是否仍在描述互動）

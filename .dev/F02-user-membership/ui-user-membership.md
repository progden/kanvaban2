...
- 帳號 ID 與系統中既有帳號重複時確認建立帳號，觸發 `uc-create-user`，輸入內容保留、顯示訊息
...
### 進入與離開
- 從哪裡進來：建立帳號成功後（依 `s-signup`「完成後去哪裡」，導向本畫面）；登出後（登出成功回到登入頁面，見本畫面操作表「登出」）；未登入時的應用程式入口（模組入口）
...
| 操作 | 觸發 | 成功後 | 失敗時 | 需確認？ |
|---|---|---|---|---|
| 送出登入表單 | `uc-login` | 依「完成後去哪裡」導向下一畫面，TopBar 顯示該使用者的 `user.display-name` | 依 `uc-login` p1／p2：帳號 ID 與密碼欄位不變，顯示訊息，停留本畫面 | 否（帳號密碼錯誤可重新輸入再次嘗試，非不可逆操作） |
...
### 驗收條件
- 帳號密碼正確時送出登入表單，觸發 `uc-login`，成功後 TopBar 顯示該使用者的 `user.display-name`（未指定顯示名字的帳號，顯示的值等於 `user.username`）
- 帳號不存在時送出登入表單，帳號 ID 與密碼欄位保留、顯示訊息、停留本畫面，且 TopBar 不顯示任何使用者的 `user.display-name`
- 密碼與帳號不相符時送出登入表單，帳號 ID 與密碼欄位保留、顯示訊息、停留本畫面，且 TopBar 不顯示任何使用者的 `user.display-name`
...
### 進入與離開
- 從哪裡進來：F07 s-canvas 上「看板成員」item 選擇加入成員的操作（依 OQ-45；機制細節待 T7.02 撰寫 s-canvas 時定案）
- 完成後去哪裡：邀請、變更角色、移除成員完成後停留本畫面，成員清單更新
- 中途放棄會怎樣：關閉本畫面，回到 F07 s-canvas，成員清單不變

### 角色與權限
| 角色 | 看得到 | 做得到 |
|---|---|---|
| `r-board-owner` | 完整成員清單（帳號、顯示名稱、角色） | 邀請成員、變更成員角色、移除成員 |
| `r-board-member` | 完整成員清單（同上） | 嘗試邀請成員、嘗試變更成員角色 |
...
### 進入與離開
- 從哪裡進來：F05 `s-workload-dashboard`（跨模組）的「點擊成員工作量查看卡片清單」操作（依 OQ-51）
- 完成後去哪裡：不適用（本畫面為唯讀清單，沒有提交或完成流程）
- 中途放棄會怎樣：關閉本畫面，回到 F05 `s-workload-dashboard`（跨模組）
...
`spec-user-membership.md`「檢視看板活動紀錄」Feature 明文標注：「Board、每張 Card、BoardMembership 的事件合併成一份、依時間排序的統一列表……這件事本質上是跨 aggregate 的查詢投影，留給『kanban-spring』有 application／persistence 層時再做……本情境目前尚未實作」。依 T2.10 分支規則，本畫面暫緩定案，僅保留八段標題與簡短說明，不產出完整內容，見 OQ-29。

### 目的
不適用，`uc-view-board-activity-log` 本情境尚未實作，暫不推導畫面目的。
...
### 待確認事項
- ⚠️ 依 spec 標注尚未實作，暫緩定案；待「kanban-spring」有跨 aggregate 查詢投影後再回頭定案，見 OQ-29
